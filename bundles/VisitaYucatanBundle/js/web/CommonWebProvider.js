...
(function () {
    ...
    app.factory('WebService', function($http, $q){
        ...
        service.payProduct = function (idVenta, numberCard, month, year, code, amount) {
            var data = "version=34&order.id=VIYUCA"+idVenta+"&transaction.id="+idVenta+"&apiOperation=PAY&sourceOfFunds.type=CARD&" +
                "sourceOfFunds.provided.card.number="+numberCard.replace(/\s/g, "")+"&sourceOfFunds.provided.card.expiry.month="+month+"&sourceOfFunds.provided.card.expiry.year="+year+"&" +
                "sourceOfFunds.provided.card.securityCode="+code+"&order.amount="+parseFloat(amount).toFixed(2)+"&order.currency=MXN";
        ...
        service.findFechasCerradasHotel = function(idHotel){
            return $http.get(service.contextPath+'/hotel/find/fechasCierreHotel/'+idHotel).success(function(data){
                arrayFechasCierres = data;
                initDatePackage();
            });
        };
